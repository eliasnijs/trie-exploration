...
internal void m_arena_init(struct mem_arena *a, void *backing_buffer,
			   size_t backing_buffer_length);
internal void * m_arena_alloc(struct mem_arena *a, size_t len);
internal void * m_arena_alloc_align(struct mem_arena *a, size_t len,
				    size_t align);
internal void m_arena_reset(struct mem_arena *a);
internal void m_arena_free(struct mem_arena *a, void *ptr);

/* function implementations */
internal void
m_arena_init(struct mem_arena *a, void *backing_buffer,
	     size_t backing_buffer_length)
{
...
m_arena_reset(struct mem_arena *a)
{
  a->offset_curr = 0;
  a->offset_prev = 0;
}
...
internal void *
m_arena_alloc(struct mem_arena *a, size_t len)
{
  return m_arena_alloc_align(a, len, MEM_DEFAULT_ALIGNMENT);
}

internal void *
m_arena_alloc_align(struct mem_arena *a, size_t len, size_t align)
{
  void *ptr = 0x0;
