...
internal void trie_init_wmem(struct trie *t, size_t l);
...
global_variable const struct trie TernaryTrieModel = {
	...
	(trie_init_wmem_fptr)ternarytrie_init_wmem,
...
internal void
trie_init_wmem(struct trie *t, size_t l)
{
	t->t = t->init_wmem(l);
}
